...
if(!isset($my_title))
{
	$my_title = 'Подача Заявки на Получение Займа Онлайн | Сервис zaimnow.tk';
...
<main class="ex-form">
    ...
    <div class="container">
        <form id="anketa" action="/lk" method="post" class="form-horizontal" onsubmit="return validate();" autocomplete="off">
            ...
            <?php
                $ddd = $this->input->post("chatbot", TRUE);
                switch($ddd)
                {
                    case 'true': $ddd = 'true'; break;
                    default: $ddd = 'false'; break;
                }
            ?>
            <input type="hidden" id="chatbot" name="chatbot" value="<?=$ddd;?>">
