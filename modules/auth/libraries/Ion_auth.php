		...
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user)
			{
				...
				else
				{
					...
					$this->email->subject('['.$this->config->item('mail_name').'] ' . $this->lang->line('email_forgotten_password_subject'));
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			else
			{
				...
				$this->email->from($this->config->item('noreply_mail'), $this->config->item('mail_name'));
				$this->email->to($user->email);
				$this->email->subject('['.$this->config->item('mail_name').'] ' . $this->lang->line('email_new_password_subject'));
				$this->email->message($message);
	...
	public function register($username, $password, $email, $additional_data = array(), $group_name = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			...
			if ($id !== FALSE)
			{
				...
				$data = array(
					'addnl'    => $additional_data,
					'id'       => $id,
					'email'    => $email,
					'password' => $password,
				);
				
				$message = $this->load->view('auth/email/reg', $data, true);

				$this->email->clear();
				$this->email->from($this->config->item('noreply_mail'), $this->config->item('mail_name'));
				$this->email->to($email);
				$this->email->subject('['.$this->config->item('mail_name').'] Регистрация');
				$this->email->message($message);
				$this->email->send();
		...
		else
		{
			...
			else
			{
				...
				$this->email->from($this->config->item('noreply_mail'), $this->config->item('mail_name'));
				$this->email->to($user->email);
				$this->email->subject('['.$this->config->item('mail_name').'] ' . $this->lang->line('email_activation_subject'));
				$this->email->message($message);
