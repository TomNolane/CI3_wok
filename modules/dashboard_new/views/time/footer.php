	...
	<script>
        ...
        $('#daterange').on('apply.daterangepicker', function(ev, picker) {
            location.href = 'http://edenga.ru/dashboard_new/time/'+picker.startDate.format('YYYY-MM-DD')+'/'+picker.endDate.format('YYYY-MM-DD')+'/<?=$site?>';
        });        
        </script>
        <script>
            var lineChartData = {
                ...
                datasets : [
                    {
                        ...
                        data : [
                            <?php foreach(array_reverse($data) as $item) { ?>
                                <?='"'. round($item['data']['time1'] / $item['data']['count']) .'",'?>
                            <?php } ?> 
                        ]
                    },
                    {
                        ...
                        data : [
                            <?php foreach(array_reverse($data) as $item) { ?>
                                <?='"'. round($item['data']['time2'] / $item['data']['count']) .'",'?>
                            <?php } ?> 
                        ]
                    },              
                    {
                        ...
                        data : [
                            <?php foreach(array_reverse($data) as $item) { ?>
                                <?='"'. round($item['data']['time3'] / $item['data']['count']) .'",'?>
                            <?php } ?> 
                        ]
                    }          
                ]
