...
(function( $, undefined ) {
	...
	$.Gallery.prototype 	= {
		_init 				: function( options ) {
			...
			this.$indicators    = this.$el.find('.dg-indicators').children();
		...
		_layout				: function() {
			...
			if( this.support3d && this.supportTrans ) {
				...
				leftCSS 	= {
					'-webkit-transform'	: 'translateX(-120px) translateZ(-200px)',
					'-moz-transform'	: 'translateX(-120px) translateZ(-200px)',
					'-o-transform'		: 'translateX(-120px) translateZ(-200px)',
					'-ms-transform'		: 'translateX(-120px) translateZ(-200px)',
					'transform'			: 'translateX(-120px) translateZ(-200px)'
				};
				...
				leftCSS.opacity		= 0.25;
				leftCSS.visibility	= 'visible';
				rightCSS.opacity	= 0.25;
		...
		_loadEvents			: function() {
			...
			this.$indicators.on('click', function(event){
				_self._goto($(event.currentTarget).data('slide-to'));
			});
		...
		_getCoordinates		: function( position ) {
			...
			if( this.support3d && this.supportTrans ) {
				...
				switch( position ) {
					case 'outleft':
						return {
							'-webkit-transform'	: 'translateX(-120px) translateZ(-300px)',
							'-moz-transform'	: 'translateX(-120px) translateZ(-300px)',
							'-o-transform'		: 'translateX(-120px) translateZ(-300px)',
							'-ms-transform'		: 'translateX(-120px) translateZ(-300px)',
							'transform'			: 'translateX(-120px) translateZ(-300px)',
							'opacity'			: 0,
							'visibility'		: 'hidden'
						};
						break;
				...
				switch( position ) {
					case 'outleft':
						return {
							'-webkit-transform'	: 'translate(-120px) scale(0.7)',
